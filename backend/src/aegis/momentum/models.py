...
`trailing_activation_pct` arms the trailing leg only once price has moved
that much in profit - without it, Binance arms trailing from the entry
price immediately, so the very first tick of normal noise can trigger a
loss before the trade ever had a chance to run (found live: two real
entries both closed via TRAILING_STOP within under an hour, small losses,
before any real move developed). Until activation, the hard ATR stop is the
only protection - which is the intended trade-off, giving the position room
to breathe past entry-point noise.
...
class MomentumConfig:
    ...
    trailing_callback_rate_pct: float = 2.0  # Binance's allowed range: 0.1-10
    trailing_activation_pct: float = 1.5  # % move in profit before the trailing leg arms itself
    leverage: int = 3
    warmup_bars: int = 210
